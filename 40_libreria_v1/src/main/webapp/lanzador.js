	...
	function lanzarPeticionLibros() {
		...
		$.get(url, params, function(data) {
			...
			$.each(data, function(i, libro) {
				var isbn = libro.isbn;
				tabla += `<tr class="border-2 p-2"><td>${libro.isbn}</td><td>${libro.titulo}</td><td>${libro.paginas}</td><td>${libro.precio}</td><td><a href="#" id=isbn class="text-white text-center"> Comprar </a></td></tr>`;
